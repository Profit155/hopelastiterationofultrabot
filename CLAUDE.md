...
### Observation Space (234 floats)
Defined in `GameStateReader` (C#) and mirrored in `ultrakill_env.py` (Python). Layout:
- **Player state** (41): position, velocity, look direction, HP, weapon charges, style rank, etc.
- **Raycasts** (24): 12 horizontal + 4 vertical + 8 diagonal normalized distance rays
- **NavMesh hint** (5): direction/distance to next checkpoint (queried every 10 frames)
...
### Action Space (22 floats)
Defined in `ActionExecutor`. First 4 are continuous (move fwd/right, look yaw/pitch). Remaining 18 are binary (>0.5 threshold): jump, dash, slide, fire1, fire2, punch, 6 weapon slots, coin, whiplash, slam, swap variation, rail charge, noop.
