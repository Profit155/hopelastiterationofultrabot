...
namespace UltrabotMod
{
    public class TcpBridge
    {
        ...
        {
            if (!_running || !_listenerStarted) return;
        ...
        private void HandleStep()
        {
            ...
            float reward = CalculateReward(info, events);
        ...
        private float CalculateReward(RewardInfo info, StepEvents events)
        {
            float reward = 0f;

            // === Component tracking for HUD ===
            float rewStyle = events.StylePointsGained * 0.01f;
            float rewKills = events.KillsThisStep * 1.0f;
            float rewParry = events.ParriesThisStep * 3.0f;
            float rewHeadshot = events.HeadshotsThisStep * 0.5f;
            float rewDamage = events.DamageTakenThisStep * -0.5f;
            float rewDeath = info.Dead ? -10f : 0f;
            ...
            // Survival
            float rewSurvival = 0.002f * (info.Hp / 100f);

            // === Exploration reward (horizontal only — no Y to prevent air abuse) ===
            float rewExplore = 0f;
            float rewHeight = 0f;
            var player = UnityEngine.Object.FindObjectOfType<NewMovement>();
            if (player != null)
            {
                ...
                delta.y = 0; // ignore vertical — only horizontal progress counts
                float dist = delta.magnitude;
                _lastExploreDist = dist;
                if (dist > _maxDistFromSpawn + 1f) // 1 unit threshold to avoid jitter
                {
                    rewExplore = (dist - _maxDistFromSpawn) * 0.05f;
                    _maxDistFromSpawn = dist;
                }

                // Height penalty — discourage flying/staying high above spawn
            ...
            reward = rewStyle + rewKills + rewParry + rewHeadshot + rewRank
                   + rewSurvival + rewDamage + rewDeath + rewExplore + rewHeight
                   + rewJitter + rewWasted + rewBank + rewSlamAbuse
                   + rewSwitchSpam + rewWhipSpam + rewFireTog + rewMash
                   - 0.001f;
